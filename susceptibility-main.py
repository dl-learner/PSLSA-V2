...
class Stats(QMainWindow):
    ...
    def __init__(self):
        ...
        # self.initUI()
        # 从文件中加载UI定义
        self.ui = uic.loadUi("PSLSA-GUI.ui")

        #设置图标 及 主标题
        self.ui.setWindowIcon(QIcon('icon.ico'))
        self.ui.setWindowTitle('Landslide susceptibility assessment system')
        # 信号和槽进行连接、初始化变量
...
#函数1，用来写入高程、坡度、破面、剖面曲率 文件路径 及 进行数据转换
    ...
    def checkVIF_new(self):
        ...
        #sd2 = pd.read_csv('D:/pycharm2022/landslide-dam/imputation acc test/ori_data_x.csv', header=None)
        selected_factors = list(sd2.columns)
        print(sd2.head())
        del selected_factors[-1]
        fac = {}
        for i in range(len(selected_factors)):
            xx = {i: sd2[selected_factors[i]]}
            fac.update(xx)
            # print(fac)
        df = pd.DataFrame(fac)
        # df['c'] = 1
        ...
        for i in range(len(VIF['feature'])-1):
            self.ui.textBrowser_22.append(str(selected_factors[i]) + ': ' + str(round(VIF['VIF'][i], 3)))
